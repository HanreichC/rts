...
public partial class HexGrid : Node3D
{
    ...
    private void AddHexTile(
        PackedScene hexScene,
        Vector2I key)
    {
        if (_tiles.ContainsKey(key))
            return;

        var hex = InstantiateHexTile<HexTileBase>(hexScene);
        if (hex == null)
            return;
        
        hex.TryPlace(key, HexSize);
        AddChild(hex);
    ...
    private void AddGhostHexTile(Vector2I key)
    {
        ...
        var ghost = InstantiateHexTile<GhostHexTile>(GhostHexTileScene);
        if (ghost == null)
            return;

        ghost.TryPlace(key, HexSize);
        AddChild(ghost);
        
        _ghostTiles[key] = ghost;
    }

    private static T InstantiateHexTile<T>(PackedScene scene) where T : HexTileBase
        => scene.Instantiate() is not T hexTile ? null : hexTile;
